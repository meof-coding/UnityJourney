    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (input[0] != 'q')
            {
                ...
                // comments as needed
                string result = "";
                //Count how many times each alphabetic character appears in input
                ...
                for (int i = 0; i < input.Length; i++)
                {
                    ...
                    if (char.IsLetter(c))
                    {
                        int index = ConvertCharToIndex(char.ToUpper(c));
                        charCount[index]++;
                    }
                }

                //convert each element in charCount to a character
                char[] charArray = new char[26];
                for (int i = 0; i < charCount.Length; i++)
                {
                    if (charCount[i] > 0)
                    {
                        charArray[i] = ConvertIndexToChar(i);
                        result += $"{charArray[i]}{charCount[i]} ";
                    }
                }
